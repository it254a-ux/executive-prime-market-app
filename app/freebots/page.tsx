    ...
    <main style={{ margin: 0, padding: 0, width: '100vw', height: '100vh', background: '#0a0a0a', fontFamily: 'Inter, sans-serif', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <aside style={{ width: sidebarOpen ? '200px' : '52px', flexShrink: 0, background: '#0f0f0f', borderRight: '1px solid rgba(201,168,76,0.12)', display: 'flex', flexDirection: 'column', padding: '12px 6px', gap: '2px', zIndex: 50, transition: 'width 0.2s ease', overflow: 'hidden' }}>
          ...
          {navLinks.map(link => (
            <a key={link.label} href={link.href} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 10px', borderRadius: '8px', color: link.href === '/freebots' ? '#c9a84c' : 'rgba(255,255,255,0.6)', fontSize: '13px', textDecoration: 'none', background: link.href === '/freebots' ? 'rgba(201,168,76,0.1)' : 'transparent', borderLeft: link.href === '/freebots' ? '2px solid #c9a84c' : '2px solid transparent', whiteSpace: 'nowrap', overflow: 'hidden' }}
            onMouseEnter={e => { if(link.href !== '/freebots') { e.currentTarget.style.color='#c9a84c'; e.currentTarget.style.background='rgba(201,168,76,0.06)'; }}}
            onMouseLeave={e => { if(link.href !== '/freebots') { e.currentTarget.style.color='rgba(255,255,255,0.6)'; e.currentTarget.style.background='transparent'; }}}>
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '44px', background: '#0d0d0d', borderBottom: '1px solid rgba(201,168,76,0.08)', display: 'flex', alignItems: 'center', paddingLeft: '20px', zIndex: 5 }}>
            <span style={{ color: '#c9a84c', fontSize: '14px', fontWeight: 600 }}>🎁 Free Bots by EPM</span>
          </div>
          <div style={{ position: 'absolute', top: '44px', left: 0, right: 0, bottom: 0, overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', bottom: '50px', right: '16px', zIndex: 20 }}>
              <a href="https://app.deriv.com/bot" target="_blank" rel="noreferrer" style={{ fontSize: '11px', color: 'rgba(201,168,76,0.5)', textDecoration: 'none' }}>Open in new tab ↗</a>
            </div>
          </div>
        </div>
      </div>
